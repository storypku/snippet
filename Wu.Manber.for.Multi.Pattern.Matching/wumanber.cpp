 ...
 */
int WuManber::Search(const char* text, const int textLength, ResultPattPosMap& res)
{
                ...
                if (prefixHash == iter->first)
                {
                    ...
                    if ('\0' == *indexPattern) {
                        res.insert(std::make_pair(
                                    string(mPatterns[iter->second]), index - mMin +1)
                                  );
                        ++hits;
                    }
                }//end if
 ...
 * Search
 */
int WuManber::Search(const string& str, ResultPattPosMap& res)
{
